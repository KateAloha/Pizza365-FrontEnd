...
//Import action types
import { fetchVoucher, createOrder } from "../../actions/FetchActionTypes";
...
function SendOrderComponent (
    {fullnameProps, emailProps, phoneNumberProps, addressProps, maGiamGiaProps, messageProps}
    ){
    ...
    const onBtnConfirmOrder = () => {
        ...
        fetchApiConfirmOrder(BASE_URL_ORDERS, body)
            .then((data) => {
                ...
                // closeConfirmOrderModal(); //đóng modal confirm order
            })
            .catch((error) => console.error(error.message))
    }
